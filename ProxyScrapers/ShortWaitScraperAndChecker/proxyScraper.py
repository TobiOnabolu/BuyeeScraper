...
async def scrape(method, output, verbose):
    ...
    if method == "socks":
        ...
        methods += ["socks4", "socks5", "http", "https"]
    ...
    proxies = set() # Initialize a set to store proxies

    # Load already stored proxies from allProxies.txt. Use a+ incase file does not exist, so it can be created
    try:
    ...
    async def scrape_scraper(scraper):
        try:
            ...
            new_proxies = await scraper.scrape(client)
            for proxy in new_proxies:
                proxies.add(proxy) # Add new proxies to the set
        except Exception:
            pass
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="../allProxies.txt", # Move up a directory to get allProxies files
    )
    parser.add_argument(
        "-v",
        "--verbose",
    ...
    # Hold all already stored proxies so that we are not storing duplicates
